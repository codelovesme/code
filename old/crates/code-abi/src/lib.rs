//! `code-abi` — the single source of truth for the Code native-module C ABI.
//!
//! Both the host runtime (the `code` crate: `native_module.rs`, `wasm_module.rs`)
//! and the module-authoring helper crate (`code-native`) depend on this crate so
//! the ABI version, tag constants, and `#[repr(C)]` struct layouts cannot drift
//! apart. The C mirror of these definitions lives in
//! `tests/native_modules/code_abi.h` and must be kept in sync by hand.
//!
//! This crate has no dependencies and no host-specific logic, so it links
//! cleanly into standalone native modules built with `code-native`.
