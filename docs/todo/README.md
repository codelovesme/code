...
| Task | Why it matters |
|---|---|
| [errors-for-absent-constructs.md](errors-for-absent-constructs.md) | **Shipped 2026-09-03.** `fn`, `while`, `for`, `else`, `print`, `import`, `class` and their synonyms are answered by name — what does not exist, and the shape that replaces it — instead of `expected '=' or '+=' after 'fn'`, which said the *assignment* was malformed and left the largest thing about this language to be discovered. None of the words became reserved: the hint fires only where an assignment was expected and did not arrive, so `print = 1` still binds a variable called `print`. `let`, `var`, `const` and `export` joined the list on 2026-09-09, when they stopped being part of the language. `tests/absent_constructs.rs` tests the wording, because the `fail_` fixtures pass either way |
| [silent-typos-at-the-particle-boundary.md](silent-typos-at-the-particle-boundary.md) | **Shipped 2026-09-11.** `code check` reports statically knowable unknown local handlers and fields as structured diagnostics; `code run --strict` and `code build --strict` apply the same analysis before execution or artifact creation. Ordinary execution remains permissive and dynamic particles remain runtime-dispatched |
| [ai-first-handlers.md](ai-first-handlers.md) | Make the one handler abstraction discoverable to coding agents without adding functions or another call syntax: begin with a deterministic JSON handler catalog, then build validation, diagnostics, tracing, and capability metadata on that surface |
| [build-targets.md](build-targets.md) | Phases 1 and 2 shipped 2026-08-24: `--target exe\|shared\|static\|wasm` works; wasm uses a small freestanding runtime shim and host-supplied clock/error imports, with Node coverage in `tests/build_targets.rs`. **Phase 3 shipped 2026-08-30**: `shared`/`static` now emit *module-ABI libraries* — a `.code` file's handlers become `code_module_dispatch`, so another `.code` program links it exactly as it links a C or Rust module. (Its `export let`s became `code_module_vars` until `export` was removed on 2026-09-09; a `.code` library reports no values now, and does not emit that entry point at all.) This reverses the document's own recommendation, which had argued for a separate `--lib`; the premise it rested on ("this language has no handler-definition syntax") stopped being true. The stream moved into a private `_code_init` behind one guarded `_code_lazy_init`, because a consumer reads `code_module_vars` at `link` time, before it has dispatched anything. Two bugs found on the way: **a library must sweep nothing** — `emit_cleanup` freed every top-level slot when `_code_init` returned, which is *before* `code_module_vars` copies anything out, so an exported value was read after its block was freed (arrays survived it silently, a heap string aborted the interpreter in glibc's allocator, a two-value module answered wrong) — and **an archive must hide its own internals**, since both sides of a static link generate `_code_init`, `_code_dispatch_this` and `_code_slot_0_num` alike. One characteristic kept: a `.a`'s exported heap values are counted by a leak-checked host, which is the `.a` contract showing through. Covered by `tests/library_targets.rs`, which links a `code`-built module from `code` |
| [community-modules.md](community-modules.md) | How native modules reach users: core stays minimal (`Length`, `Timestamp`, `Linked`), first-party modules are per-host (`console` native / `console` browser, then `math`, `strings`, `http_client`) on GitHub Releases + npm via `code install`, and the community publish path — direction decided 2026-08-23, phased A–F |
...
Cleaned up 2026-09-10. Two files that were only history came out —
`single-line-blocks.md`, whose whole subject went with the braces, and
`hosting-a-guests-modules.md`, which `README.md` and `tests/hosted_app.rs`
between them now say better. The rest of the "shipped" entries stay: comments
in the source point at them for the *why*, and a dangling pointer costs more
than a closed file does. Where the language has since moved under one of those
write-ups it now says so at the top, rather than being rewritten — what was
true when it was written is worth keeping legible.

Done and removed (git log has the detail):

- *single-line blocks* — the whole subject went with the braces on
  2026-09-09. A block is the indented run of lines under its header, and the
  guard run it existed for is now `if score ≥ 90, return G { letter = "A" }`,
  the comma being the same one that separates two of anything else on a line.
- *hosting a guest's modules* — shipped 2026-09-04 and closed; `README.md`
  documents what a host offers a guest, and `tests/hosted_app.rs` holds it.
...
- *user-defined handlers* — `ClassName { fields } =>` with an indented body — braces
  at the time — and `emit … to this`, in both backends. Two claims in the old write-up turned
