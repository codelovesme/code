...
| [inbound-emissions-from-native-modules.md](inbound-emissions-from-native-modules.md) | Phases A and B shipped 2026-08-25: a module exports `code_module_set_inbound` and pushes particles, both modes drain between top-level statements and dispatch to the *program's* handlers, bounded at 256 dropping oldest. `.a` static modules joined on 2026-08-28 (`code_static_open` gives one the queue its missing `dlopen` handle used to hold). A loop iteration became a statement boundary on 2026-08-29, so a polling event loop works and `loop { }` is how a program says "keep me up" (owner's call). Still open: a module pushing from a thread of its own — which needs a lock on the ring, an atomic leak counter, and a sleep for an empty wait loop. Plus the old `emit … to base` — an *upward* edge in the module graph |
