...
| [errors-as-particles.md](errors-as-particles.md) | Decided 2026-08-28, not yet built: a module may never end the application, an unhandled `to <alias>` yields null, and `assert` becomes `emit Exception` + `return`. Reverses the README's "no catchable asserts". Two tensions still open (what an unhandled `Exception` does; what a refusing module returns) and 76 `fail_*` fixtures ride on getting it right |
