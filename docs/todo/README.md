...
| Task | Why it matters |
|---|---|
| [single-line-blocks.md](single-line-blocks.md) | `if score ≥ 90 { return G { letter = "A" } }` does not parse — a statement inside a block needs a `;` or a newline before the `}`, so the guard clause, the shape the missing `else` makes idiomatic here, is the one shape the syntax refuses. An object literal has the opposite rule for the same brace (`let o = { a = 1 }` is fine inline, and the formatter preserves it), the restriction is undocumented, and the error (`expected end of statement, found RBrace`) names neither the cause nor the fix. Found by writing a first program cold; hit twice in twenty minutes |
