//! End-to-end tests for the `code format` CLI command.

use std::fs;
use std::process::Command;

fn tmp(name: &str) -> std::path::PathBuf {
    std::env::temp_dir().join(format!("code_format_{}_{}.code", std::process::id(), name))
}

#[test]
fn format_check_then_format_in_place() {
    ...
    let st = Command::new(exe).args(["format", p, "--check"]).status().unwrap();
    ...
    let st = Command::new(exe).args(["format", p]).status().unwrap();
    assert!(st.success());
    assert_eq!(
...
#[test]
fn format_directory_recurses() {
    let exe = env!("CARGO_BIN_EXE_code");
    let dir = std::env::temp_dir().join(format!("code_format_dir_{}", std::process::id()));
...
#[test]
fn format_missing_file_argument_errors() {
    let exe = env!("CARGO_BIN_EXE_code");
    let st = Command::new(exe).arg("format").status().unwrap();
    assert!(!st.success(), "`code format` with no file should error");
}
