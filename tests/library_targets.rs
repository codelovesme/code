...
/// A `.so` exports its ABI entry points and nothing of its own.
///
/// The `.a` half of this rule has been enforced since libraries shipped; the
/// `.so` half was not, and every `.code` library was handing out its whole
/// inside: `_code_init`, `_code_dispatch_this`, `_code_module_initialized`,
/// one symbol per top-level slot. A `.so` has its own symbol namespace, which
/// is why nothing had broken *yet* — but exporting a module's initialisation
/// guard and handler chain by name is an invitation for two libraries in one
/// process to answer for each other, and the names are generated identically
/// in every one of them.
///
/// The runtime functions (`code_release`, `code_native_open`, ...) stay
/// exported: they come from the C runtime linked in alongside, not from the
/// generated object, and a `.so` is required to carry its own copy.
#[test]
fn a_shared_library_exports_only_its_abi_entry_points() {
    let dir = temp_dir("so_symbols");
    let library = build(
        &dir,
        "lib",
        MODULE_SOURCE,
    ...
    let listing = Command::new("nm")
        ...
        .output()
        .expect("run nm");
    assert!(listing.status.success(), "nm failed on the library");
    let leaked: Vec<String> = String::from_utf8_lossy(&listing.stdout)
        .lines()
        .filter_map(|line| line.split_whitespace().last())
        .filter(|name| name.starts_with("_code_"))
        .map(str::to_owned)
        .collect();

    assert!(
        leaked.is_empty(),
        "a .so should keep its internals to itself, but exports: {leaked:?}"
    );

    let listing = String::from_utf8_lossy(&listing.stdout);
    for entry in ["code_module_abi_version", "code_module_dispatch"] {
        assert!(
            listing.contains(entry),
            "a .so must still export '{entry}':\n{listing}"
        );
    }
...
/// Two `.code` libraries in one program each answer for themselves.
///
/// Both are generated by the same compiler from the same shapes, so every
/// internal name in one is also in the other. This is the property that
/// depends on the test above, and the reason it is worth having: a program
/// that hosts more than one application is exactly a program with two of
/// these loaded at once.
#[test]
fn two_shared_libraries_do_not_answer_for_each_other() {
    let dir = temp_dir("two_libs");
    build(
        &dir,
        "one",
...
assert first.who = "one"
assert second.who = "two"
"#,
    )
    .expect("write consumer");
    ...
    assert!(
        interpreted.status.success(),
        "interpreted: two libraries answered for each other:\n{}",
        stderr_of(&interpreted)
    );
    ...
    assert!(
        compiled.status.success(),
        "compiled: two libraries answered for each other:\n{}",
        stderr_of(&compiled)
    );
