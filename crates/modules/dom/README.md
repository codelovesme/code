...
- **`reorder`** — on a *container*: a child pressed, carried 8px or more —
  up or down; a press that sets off sideways is left to the child, so a
  `swipeleft` on it still lands — and let go among its siblings. Sent with **`from`** and **`at`**, the child's
