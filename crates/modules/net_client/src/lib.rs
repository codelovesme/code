...
//!
//! **In a browser it does not answer.** Waiting means blocking, and blocking
//! in a page freezes the reader — so the browser half answers `SendResult {
//! ok, value }` as soon as the request is on its way, and the reply arrives
//! later as a particle carrying `_request_id`. See this module's README.
//!
//! **On a machine `Send` blocks**, like `http_client`'s handlers do, and
//! bounds the block:
...
mod page {
    ...
    browser_half!(
        "net_client",
        net_client_code_module_abi_version,
        net_client_code_module_dispatch
    );
}
