...
//! ## In a browser
//!
//! Built for wasm the module is the same two handlers with `JSON` behind
//! them — its `page.mjs`, which `code build --target wasm` writes into the
//! page's `host.mjs` — so a page can read back what `storage` kept for it.
//! The wasm half is `no_std` and hands the particle over, like every browser
//! module's: see `crates/modules/browser_half.rs`.
//!
//! ```bash
...
mod page {
    ...
    browser_half!(
        "json",
        json_code_module_abi_version,
        json_code_module_dispatch
    );
}
